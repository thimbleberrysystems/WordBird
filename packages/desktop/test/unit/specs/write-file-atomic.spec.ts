...
// Saves must survive an application crash AND a power loss. writeFile writes to
// a temp file, fsyncs it, then renames it over the target (via
// write-file-atomic), so an interrupted or unflushed write can never leave a
// manuscript truncated or zero-filled — and, unlike a plain temp+rename, the
// target's permission mode is preserved across the save.
//
// Ported from upstream marktext 6c23b1ba.

const dirs: string[] = []
function tempDir(): string {
  const d = mkdtempSync(path.join(tmpdir(), 'wb-atomic-'))
  dirs.push(d)
  return d
}
...
describe('writeFile — durable atomic save', () => {
  ...
  it('still recreates a missing parent directory', async() => {
    // A save whose folder was moved or deleted must recreate it and succeed,
  ...
  it('appends the extension when the path lacks it', async() => {
    const dir = tempDir()
    const target = path.join(dir, 'note')

    await writeFile(target, 'x', '.md')

    expect(readFileSync(`${target}.md`, 'utf-8')).toBe('x')
  })

  it('rejects a save with no path rather than throwing synchronously', async() => {
    await expect(writeFile('', 'x')).rejects.toThrow(/without path/)
  })
