...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to WordBird and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'wordbird-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: wordbird [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`WordBird: ${MARKTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'wordbird-user-data')
