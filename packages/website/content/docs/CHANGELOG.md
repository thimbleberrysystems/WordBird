...
Mark Text is now WordBird! We'd like to thank all contributors and users that have been using WordBird and worked on this release.
...
- WordBird is now single-instance application on Linux and Windows too.
...
- WordBird can use a default directory that is automatically opened during startup (#711)
...
- Support maxOS `dark mode`, when you change `mode dark or light` in system, WordBird will change its theme.
...
- Restore default WordBird style after exporting/printing
...
- WordBird didn't remove highlight when I delete the markdown symbol like \* or `. (#893)
...
You need uninstall the old version of WordBird before install version 0.10.21, because we changed the AppId when build.
...
- Add user preferences in `WordBird menu`, the shoutcut is `CmdorCtrl + ,`, you can set the default `theme` and `autoSave`.
- Add `autoSave` to `file menu`, the default value is in `preferences.md` which you can open in `WordBird menu`. #45
- Add drag and drop to open Markdown file with WordBird @fxha
...
- fix: prevent open image or file directly when drag and drop over WordBird #42
