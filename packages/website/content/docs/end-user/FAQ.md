...
WordBird is a desktop application and available for:
...
WordBird is a pure markdown editor without feature such as knowledge management and tags but yes, you can do this via the integrated filesystem explorer and task lists.
...
### Can I run a portable version of WordBird?
...
### I cannot launch WordBird on Linux (SUID sandbox)

> _The SUID sandbox helper binary was found, but is not configured correctly._

Normally, you should never get this error but if you disabled user namespaces, this error message may appears in the command output when launching WordBird. To solve the issue, that Chromium cannot start the sandbox (process), you can choose one of the following steps:
...
- Launch WordBird with `--no-sandbox` argument.
