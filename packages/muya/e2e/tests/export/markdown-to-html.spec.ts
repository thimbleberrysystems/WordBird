...
test.describe('export / MarkdownToHtml', () => {
  ...
  test('script injection: <script> tag is sanitised away and does not execute', async ({
    page
  }) => {
    ...
    const sideEffect = await page.evaluate((generated) => {
      ...
      if (!articleMatch) return { mounted: false, exported: undefined as boolean | undefined }
