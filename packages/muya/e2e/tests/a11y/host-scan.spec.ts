...
function logNonCritical(scope: string, violations: ReadonlyArray<IAxeViolation>): void {
  if (violations.length > 0) {
    ...
    console.log(
      `[a11y/${scope}] non-critical violations:`,
...
test.describe('a11y / host page scan', () => {
  test('clean host page has no critical violations after init', async ({ page }) => {
    ...
    expect(
      critical,
      critical.length ? `critical: ${critical.map((v) => v.id).join(', ')}` : ''
    ).toEqual([])
  })
  ...
  test('image toolbar visible after clicking an image: no critical violations', async ({
    page
  }) => {
    await page.evaluate(() => {
  ...
  test('table tools visible after clicking into a table cell: no critical violations', async ({
    page
  }) => {
    await page.evaluate(() => {
