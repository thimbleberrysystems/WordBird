...
test.describe('edges / empty and tiny documents', () => {
  test('setContent("") leaves a single empty paragraph and no crash', async ({ page }) => {
    ...
    const state = await page.evaluate(
      () => window.muya!.getState() as Array<{ name: string; text?: string }>
    )
  ...
  test('10× rapid setContent without awaiting — final state matches the last call', async ({
    page
  }) => {
    ...
    await page.evaluate(() => {
      for (let i = 0; i < 10; i++) window.muya!.setContent(`# round ${i}`)
    })
