...
test.describe('stability / listener leak', () => {
  test('50× setContent/locale/destroy/init cycle keeps listener count bounded', async ({
    page
  }) => {
    ...
    const baseline = await page.evaluate(() => ({
      ...
      listenerTotal: Object.values(window.muya!.eventCenter.listeners).reduce(
    ...
    expect(
      delta,
      `listener total grew by ${delta} over 49 rebuilds (iter1=${afterIter1.listenerTotal}, iter50=${afterIter50.listenerTotal})`
    ...
    expect(
      domDelta,
      `domEvents grew by ${domDelta} (iter1=${afterIter1.domEvents}, iter50=${afterIter50.domEvents})`
    ).toBeLessThanOrEqual(5)
