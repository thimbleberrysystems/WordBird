...
test.describe('stability / perf smoke @perf', () => {
  ...
  test('setContent with 10k paragraphs finishes within the budget and scroll is reachable', async ({
    page
  }) => {
    ...
    const result = await page.evaluate(() => {
      const N = 10_000
      const lines: string[] = []
      for (let i = 0; i < N; i++) lines.push(`paragraph ${i}`)
      const md = lines.join('\n\n')
    ...
    expect(
      result.ms,
      `setContent(${result.n} paragraphs) took ${result.ms.toFixed(0)}ms`
    ).toBeLessThan(60_000)
