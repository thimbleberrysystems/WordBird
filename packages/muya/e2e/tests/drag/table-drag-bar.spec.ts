...
test.describe('TableDragBar column reorder', () => {
  test('drag bar appears when hovering just below the table', async ({ page }) => {
    ...
    if (!box) throw new Error('header cell has no bounding box')
  ...
  test('dragging the first column past the second swaps column order in markdown', async ({
    page
  }) => {
    ...
    if (!firstBox || !secondBox) throw new Error('last-row cells have no bounding box')
    ...
    if (!barBox) throw new Error('drag bar has no bounding box')
