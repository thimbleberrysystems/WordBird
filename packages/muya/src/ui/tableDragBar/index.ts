...
export function getIndex(barType: BarType, cellBlock: TableBodyCell) {
  ...
  return barType === 'bottom' ? row.offset(cellBlock) : (table.firstChild as TableInner).offset(row)
}
...
export class TableDragBar extends BaseFloat {
  ...
  startDrag(event: Event) {
    ...
    for (const row of this._dragInfo.cells) {
      for (const cell of row) {
        if (!this._dragInfo.dragCells.includes(cell)) cell.classList.add('mu-cell-transform')
      }
    }
    ...
    if (offset > 0) {
      if (barType === 'bottom') {
        for (const row of cells) {
          for (i = 0; i < len; i++) {
            const cell = row[i]
            if (i > index && i <= curIndex) cell.style.transform = `translateX(${-aspect}px)`
        ...
        for (i = 0; i < len; i++) {
          ...
          for (const cell of row) {
            if (i > index && i <= curIndex) cell.style.transform = `translateY(${-aspect}px)`
      ...
      if (barType === 'bottom') {
        for (const row of cells) {
          for (i = 0; i < len; i++) {
            const cell = row[i]
            if (i >= curIndex && i < index) cell.style.transform = `translateX(${aspect}px)`
            else if (i !== index) cell.style.transform = 'translateX(0px)'
          }
        }
      } else {
        for (i = 0; i < len; i++) {
          ...
          for (const cell of row) {
            if (i >= curIndex && i < index) cell.style.transform = `translateY(${aspect}px)`
            else if (i !== index) cell.style.transform = 'translateY(0px)'
          }
        }
      }
  ...
  switchTableData = () => {
    ...
    if (table.active) {
      ...
      const { rowOffset, columnOffset } = anchorBlock?.closestBlock('table.cell') as TableBodyCell
