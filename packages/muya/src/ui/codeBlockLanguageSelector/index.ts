...
export class CodeBlockLanguageSelector extends BaseScrollFloat {
  ...
  override listen() {
    ...
    eventCenter.on('content-change', ({ block }) => {
      if (block.blockName !== 'paragraph.content' && block.blockName !== 'language-input') return
  ...
  override selectItem(item: { name: string }) {
    ...
    function isParagraphContent(b: ParagraphContent | LangInputContent): b is ParagraphContent {
