...
class Format extends Content {
  ...
  updateImage({ imageId, token }: IImageInfo, attrName: string, attrValue: string) {
    ...
    const selector = `#${imageId.includes('_') ? imageId : `${imageId}_${token.range.start}`} img`
  ...
  override clickHandler(event: Event): void {
    ...
    if (inlineRuleRenderEle) return this._handleClickInlineRuleRender(event, inlineRuleRenderEle)
    ...
    const footnoteEl = target.closest<HTMLElement>(`.${CLASS_NAMES.MU_INLINE_FOOTNOTE_IDENTIFIER}`)
  ...
  convertToTaskList() {
    ...
    if (!list || list.blockName !== 'bullet-list' || !parent!.isFirstChild() || matches == null) {
      return
    }
  ...
  override backspaceHandler(event: Event): void {
    ...
    for (const token of tokens) {
      ...
      if (token.range.start + 1 === start.offset && preToken && preToken.type === 'image') {
  ...
  override deleteHandler(event: KeyboardEvent): void {
    ...
    while (needRemovedBlock && needRemovedBlock.isOnlyChild() && !needRemovedBlock.isScrollPage) {
