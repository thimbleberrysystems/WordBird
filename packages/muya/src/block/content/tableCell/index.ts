...
class TableCellContent extends Format {
  ...
  override enterHandler(event: Event) {
    ...
    else if ((isOsx && event.metaKey) || (!isOsx && event.ctrlKey)) return this.commandEnter(event)
  ...
  override tabHandler(event: Event) {
    ...
    const cursorBlock = isShiftTab ? this.previousContentInContext() : this.nextContentInContext()
