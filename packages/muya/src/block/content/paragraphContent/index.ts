...
class ParagraphContent extends Format {
            ...
            list.forEachAt(offset + 1, undefined, (node) => {
              if (node.isParent()) {
                ...
                if (newListState.name === 'task-list') {
                  if (isTaskListItemState(childState)) newListState.children.push(childState)
  ...
  private _handleBackspaceInBlockQuote() {
    ...
    if (!parent!.isOnlyChild() && !parent!.isFirstChild()) return this._handleBackspaceInParagraph()
  ...
  private _checkCursorAtEndFormat(): Nullable<{ offset: number }> {
    ...
    const walkTokens = (ts: Token[]) => {
      for (const token of ts) {
        ...
        if ('children' in token && Array.isArray(token.children)) walkTokens(token.children)
      }
    }
