...
class Table extends Parent {
  ...
  queryBlock(path: TBlockPath) {
    ...
    return (
      this.firstChild as Parent & { queryBlock: (p: TBlockPath) => Parent | Content | undefined }
    ).queryBlock(path)
  }
  ...
  insertRow(offset: number) {
    ...
    if (offset > 0) (this.firstChild as TableInner).insertAfter(rowBlock, currentRow as TableRow)
