...
class History {
  ...
  private _listen() {
    this.muya.eventCenter.on(
      ...
      ({
        ...
        if (!this.options.userOnly || source === 'user') this._record(op, prevDoc)
  ...
  private _record(op: JSONOpList, doc: TState[]) {
    ...
    if (this._lastRecorded + this.options.delay > timestamp && this._stack.undo.length > 0) {
      const { operation: lastOperation, selection: lastSelection } = this._stack.undo.pop()!
    ...
    if (this._stack.undo.length > this.options.maxStack) this._stack.undo.shift()
  }
