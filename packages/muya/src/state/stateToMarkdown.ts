...
export default class ExportMarkdown {
  ...
  private _listType: (IBulletListState['meta'] | IOrderListState['meta'] | ITaskListState['meta'])[]
  ...
  convertStatesToMarkdown(states: TState[], indent = '', listIndent = ''): string {
    const result = []
    ...
    for (const state of states) {
      ...
      switch (state.name) {
        ...
        case 'task-list': {
          ...
          const bulletMarkerOrDelimiter = 'delimiter' in meta ? meta.delimiter : meta.marker

          if (lastListBullet && lastListBullet !== bulletMarkerOrDelimiter) insertNewLine = false
  ...
  serializeTextParagraph(state: IParagraphState | IThematicBreakState, indent: string) {
    const { text } = state
    const lines = text.split('\n')
  ...
  serializeListItem(state: IListItemState | ITaskListItemState, indent: string) {
    const result = []
    ...
    if (isUnorderedList) {
      ...
      if ((this._listIndentation === 'dfm' && n > 99) || n > 999999999) n = 1
    ...
    if (listIndentation === 'dfm') listIndent = ' '.repeat(4 - itemMarker.length)
    else if (listIndentation === 'number') listIndent = ' '.repeat(this._listIndentationCount - 1)
    ...
    if (name === 'task-list-item') itemMarker += state.meta.checked ? '[x] ' : '[ ] '

    result.push(`${indent}${itemMarker}`)
    result.push(
      this.convertStatesToMarkdown(children, newIndent, listIndent).substring(newIndent.length)
    )
