...
export const DEFAULT_TURNDOWN_CONFIG = {
  ...
  blankReplacement(
    ...
    if (node && node.classList.contains('mu-soft-line-break')) return LINE_BREAK
    else if (node && node.classList.contains('mu-hard-line-break')) return `  ${LINE_BREAK}`
    else if (node && node.classList.contains('mu-hard-line-break-space')) return ''
