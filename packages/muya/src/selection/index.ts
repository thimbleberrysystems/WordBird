...
class Selection {
  ...
  static getSelectionStart() {
    ...
    const startNode = node && node.nodeType === Node.TEXT_NODE ? node.parentNode : node
  ...
  get direction() {
    ...
    if (anchor === null || focus === null || !anchorBlock || !focusBlock) return 'none'
  ...
  selectAll() {
    ...
    if (isHTMLElement(activeEle) && activeEle.classList.contains('mu-content')) activeEle.blur()
  }
  ...
  private _listenSelectActions() {
    ...
    const handleMouseupOrLeave = () => {
      if (this._selectInfo.selection) this.setSelection(this._selectInfo.selection)
    ...
    const handleMousemoveOrClick = (event: Event) => {
      ...
      const { anchor, focus, anchorBlock, focusBlock, isSelectionInSameBlock, direction } =
        selection
      ...
      if (/bullet-list|order-list|task-list/.test(focusOutMostBlock.blockName)) {
        const listItemBlockName =
          focusOutMostBlock.blockName === 'task-list' ? 'task-list-item' : 'list-item'
      ...
      if (type === 'mousemove') this._selectInfo.selection = newSelection
    ...
    const handleClick = (event: Event) => {
      ...
      this.selectedImage = null
      if (imageWrapper) return this._handleClickInlineImage(event, imageWrapper)
    }
  ...
  private _handleClickInlineImage(event: Event, imageWrapper: HTMLElement) {
    ...
    const deleteContainer = isHTMLElement(target) ? target.closest('.mu-image-icon-close') : null
  ...
  private _select(startNode: Node, startOffset: number, endNode?: Node, endOffset?: number) {
    ...
    if (endNode && typeof endOffset === 'number') range.setEnd(endNode, endOffset)
  ...
  private _setCursor() {
    ...
    const anchorParagraph = anchorBlock ? anchorBlock.domNode : scrollPage?.queryBlock(anchorPath)
