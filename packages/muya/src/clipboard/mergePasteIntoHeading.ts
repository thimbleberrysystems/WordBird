...
export function mergePasteIntoHeading(
  ...
  const isHeading =
    wrapperBlock?.blockName === 'atx-heading' || wrapperBlock?.blockName === 'setext-heading'
  if (!isHeading) return states

  const first = states[0]
  if (first.name !== 'paragraph') return states

  const original = anchorBlock.text
  anchorBlock.text =
    original.substring(0, cursor.startOffset) + first.text + original.substring(cursor.endOffset)
  anchorBlock.update()
