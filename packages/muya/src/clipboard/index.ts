...
class Clipboard {
  ...
  listen() {
    ...
    const pasteHandler = (event: Event) => {
      if (isClipboardEvent(event)) this.pasteHandler(event)
    }

    merge(fromEvent(domNode, 'copy'), fromEvent(domNode, 'cut')).subscribe(copyCutHandler)
  ...
  getClipboardData() {
    ...
    const startOutBlock =
      anchorOutMostBlockOffset <= focusOutMostBlockOffset ? anchorOutMostBlock : focusOutMostBlock
    const endOutBlock =
      anchorOutMostBlockOffset <= focusOutMostBlockOffset ? focusOutMostBlock : anchorOutMostBlock
    const startBlock =
      anchorOutMostBlockOffset <= focusOutMostBlockOffset ? anchorBlock : focusBlock
    const endBlock = anchorOutMostBlockOffset <= focusOutMostBlockOffset ? focusBlock : anchorBlock
    const startOffset =
      anchorOutMostBlockOffset <= focusOutMostBlockOffset ? anchor.offset : focus.offset
    const endOffset =
      anchorOutMostBlockOffset <= focusOutMostBlockOffset ? focus.offset : anchor.offset
    ...
    if (anchorOutMostBlock === focusOutMostBlock) {
      ...
      if (/block-quote|table/.test(anchorOutMostBlock!.blockName)) {
        ...
        const listItemBlockName =
          anchorOutMostBlock!.blockName === 'task-list' ? 'task-list-item' : 'list-item'
        ...
        if (isAnyListState(listState)) {
          if (listState.name === 'task-list') {
            copyState.push({
              name: 'task-list',
              meta: listState.meta,
              children: listState.children.filter(
  ...
  cutHandler() {
    ...
    const { isSelectionInSameBlock, anchor, anchorBlock, focus, focusBlock, direction } = selection
    ...
    const startOutBlock = direction === 'forward' ? anchorOutMostBlock : focusOutMostBlock
    const endOutBlock = direction === 'forward' ? focusOutMostBlock : anchorOutMostBlock
      ...
      if (
        ...
        outBlock.forEach((item, index) => {
          if (position === 'start' && index === offset) {
            ...
            const newListItem = ScrollPage.loadBlock(state.name).create(this.muya, state)
    ...
    if (cursorBlock && cursorOffset != null) cursorBlock.setCursor(cursorOffset, cursorOffset, true)
  ...
  async pasteHandler(event: ClipboardEvent): Promise<void> {
    ...
    if (/html|text/.test(copyType)) {
      ...
      if (/\n\n/.test(markdown) && anchorBlock.blockName !== 'codeblock.content') {
        ...
        const remaining = mergePasteIntoHeading(anchorBlock, wrapperBlock, states, {
          startOffset: start.offset,
          endOffset: end.offset
        })

        if (remaining === states && start.offset !== end.offset) {
          anchorBlock.text = content.substring(0, start.offset) + content.substring(end.offset)
          anchorBlock.update()
        }
        ...
        if (originWrapperBlock?.blockName === 'paragraph') {
          ...
          if (isParagraphState(originState) && originState.text === '') originWrapperBlock.remove()
        }
        ...
        if (anchorBlock.blockName === 'language-input') markdown = markdown.replace(/\n/g, '')
