...
class Renderer {
  ...
  getClassName(outerClass: string | undefined, block: Format, token: Token, cursor: ICursor) {
    return (
      outerClass ||
      (this.checkConflicted(block, token, cursor) ? CLASS_NAMES.MU_GRAY : CLASS_NAMES.MU_HIDE)
    )
  }
