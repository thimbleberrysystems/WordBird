...
export function getAttributes(html: string) {
  ...
  for (const attr of target.getAttributeNames()) {
    ...
    if (/width|height/.test(attr) && attribute) attrs[attr] = validWidthAndHeight(attribute)
...
export function parseSrcAndTitle(text = '') {
  ...
  let title = ''
  if (rawTitle && TITLE_REG.test(rawTitle)) title = rawTitle.replace(TITLE_REG, '$2')
...
function canOpenEmphasis(src: string, marker: string, pending: string) {
  ...
  if (pending.length > 0 && pending.charAt(pending.length - 1) === markerChar) return false
...
export function validateEmphasize(
  ...
  if (SHORTER_REG.test(emphasizeText) && !CLOSE_REG.test(emphasizeText)) return false
