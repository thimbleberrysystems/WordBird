  ...
  while (src.length) {
    ...
    if (
      ...
      tokens.push({
        ...
        children: tokenizerFac(rLinkTo[1], null, inlineRules, pos + 1, false, labels, options)
      })
    ...
    if (autoLinkExtTo && top && (pos === 0 || /[* _~(]/.test(originSrc[pos - 1]))) {
      pushPending()
      tokens.push({
  ...
  const postTokenizer = (tokens: Token[]) => {
    for (const token of tokens) {
      for (const light of highlights) {
        ...
        if (highlight) {
          if (token.highlights && Array.isArray(token.highlights)) token.highlights.push(highlight)
