...
function syncOptionFormFromState() {
  ...
  for (const key of RADIO_KEYS) {
    ...
    document.querySelectorAll<HTMLInputElement>(`input[name="opt-${key}"]`).forEach((el) => {
...
function readOptionFromEvent(target: EventTarget | null) {
  ...
  if (dataKey) {
    if (target instanceof HTMLInputElement && target.type === 'checkbox') {
      ...
      if (!Number.isNaN(num)) currentOptions[dataKey] = num
    } else {
      currentOptions[dataKey] = target.value
    }
    return
  }
